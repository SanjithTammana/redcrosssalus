...
import { Box, TextField, Button, CircularProgress, ToggleButton, ToggleButtonGroup, Typography } from "@mui/material";
...
const ChatPage = () => {
  const [emergencyMessages, setEmergencyMessages] = useState([
    { id: Date.now(), sender: "bot", text: "I’m your Red Cross Emergency Assistant. First, please provide the patient’s details (age, sex, weight, allergies, medications, and any family medical history). If you haven’t already, call 911 immediately for emergencies." },
  ]);
  const [practiceMessages, setPracticeMessages] = useState([
    { id: Date.now(), sender: "bot", text: "Welcome to Practice Mode! I’m here to guide you through first aid and emergency preparedness." },
  ]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { id: Date.now() + 1, sender: "bot", text: data.response || "Error: No response." },
      ]);
    } catch (error) {
